...
- [x] **P2 · 1h** [Patroclus] Run wdc_be full catalog to widen overlap coverage — ⚠ domain for sale (Nameshift/NVA Online Advertising B.V., verified 2026-05-23); wine shop no longer exists; scraper updated to log `source_dead` DLQ + set `enabled=0` in dim_source; source retired ✓ 2026-05-23
...
## Sprint 14 — Couverture France 60 % strict (ADR-013)

> Cible : 60 % des producteurs français notables avec données vérifiées multi-source. Squelette INAO d'abord, prix/ratings ensuite, ≥2 sources obligatoires.

- [x] **P0 · 4h** [Patroclus] Ingest INAO appellation registry complet (open data agriculture.gouv.fr) → étendre `dim_appellation` à toutes les AOC/IGP françaises (Bordeaux, Champagne, Loire, Rhône, Alsace, Languedoc, Provence, Jura, Savoie, Sud-Ouest). Source officielle, pas de gate multi-source nécessaire. — [#30](https://github.com/FibSol/AchillesWines/issues/30) ✓ 2026-05-23
- [x] **P0 · 6h** [Patroclus] Producer registry expansion : scraper officiel CIVB + BIVB + Inter-Rhône + InterLoire + CIVC + CIVA + CIVL → `dim_producer` avec `allowed_appellations`. Source autoritaire ; ne pollue pas les facts. — [#31](https://github.com/FibSol/AchillesWines/issues/31) ✓ 2026-05-24
- [x] **P1 · 3h** [Hector] Ajouter colonne `coverage_tier` à `dim_producer` (`notable` | `mid` | `long_tail`) + migration. Critère : notable = présent dans ≥1 guide critique OU ≥1 retailer scrapé ; mid = présent uniquement dans registry syndicat ; long_tail = INAO seul. Permet de mesurer la cible 60 %. — [#32](https://github.com/FibSol/AchillesWines/issues/32) ✓ 2026-05-23
- [x] **P1 · 3h** [Cassandra] Renforcer le promoter : exiger ≥2 sources critiques pour `fact_rating` (équivalent du gate prix ±15 %). Sources mono → rester en `staging` avec `needs_review=1`. Tests unitaires sur le nouveau gate. — [#33](https://github.com/FibSol/AchillesWines/issues/33) ✓ 2026-05-24
- [x] **P5 · 4h** [Patroclus] 🅿 PARKING — Scraper Wine-Searcher (déjà dans `dim_source` depuis Sprint 11) — auth requise, fourchettes de prix multi-shop, cuvée + millésime + appellation. Élargit fortement la couverture cuvées. — [#34](https://github.com/FibSol/AchillesWines/issues/34) ✓ 2026-05-24
- [x] **P1 · 3h** [Patroclus] Élargir iDealwine au catalogue ventes aux enchères (historique) en plus des ventes fixes → couverture millésimes anciens. — [#35](https://github.com/FibSol/AchillesWines/issues/35) ✓ 2026-05-26
...
- [ ] **P2 · 4h** [Patroclus] Populate `bridge_wine_variety` via appellation-default grape rules + scraper composition data — [#42](https://github.com/FibSol/AchillesWines/issues/42)
- [ ] **P2 · 3h** [Patroclus] Naming cleanup wave 2: add cleanup rule for `(CB6)` / `(CB3)` carton-of-N codes in cuvée names; strip appellation suffix from `cuvee_appellation_tail` (60 entries); second-pass VdF research for 1 438 remaining mixed-portfolio entries — [#45](https://github.com/FibSol/AchillesWines/issues/45)
