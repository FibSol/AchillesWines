...
"""
Millesima.be scraper — retail price ingestion via Next.js internal JSON API.

Millesima.be is the Belgian variant of millesima.fr and uses the same
Next.js structure: buildId in __NEXT_DATA__, product data via
/_next/data/{buildId}/tous-nos-vins.html.json.

Re-uses the same parsing logic as millesima.py but targets .be domain.
"""
import hashlib
import json
import logging
...
def _build_id_cache_path() -> Path:
    ...
    return Path(data_dir) / "millesima_be_build_id.json"
...
def _fetch_build_id(client: "httpx.Client", fetch_fn=None) -> Optional[str]:
    """Fetch the Next.js buildId from the millesima.be homepage __NEXT_DATA__ block."""
    try:
        url = f"{_BASE}/"
        ...
        if cached is not None:
            ...
            _logger.warning(
                "millesima.be homepage unreachable, using cached buildId from %s", cached_at
            )
...
def _parse_product(p: dict) -> Optional[dict]:
    """Convert a Millesima.be JSON product dict to a normalised card dict."""
...
class MillesimaBeScraper(BaseScraper):
    source_code = "millesima_be"
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"millesima_be-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            ...
            if not build_id:
                result.error = "Could not fetch Next.js buildId from millesima.be homepage"
                return result
            ...
            while True:
                ...
                if resp.status_code in (403, 429):
                    msg = f"Blocked by millesima.be: HTTP {resp.status_code} on {url}"
