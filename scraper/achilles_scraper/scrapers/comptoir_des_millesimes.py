...
"""
comptoirdesmillesimes.com scraper — public catalog retail price ingestion.

Comptoir des Millésimes is a French specialist wine retailer with a
publicly accessible catalogue. Products display name, vintage, and price
in standard product card format.
"""
import hashlib
import json
import logging
...
class ComptoirDesMillesimesScraper(BaseScraper):
    source_code = "comptoir_des_millesimes"
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"comptoir_des_millesimes-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            while True:
                ...
                if resp.status_code in (403, 429):
                    msg = f"Blocked by comptoirdesmillesimes.com: HTTP {resp.status_code} on {url}"
                    ...
                    try:
                        self.conn.execute(
                            """INSERT OR IGNORE INTO staging_price_candidates
                               ...
                               VALUES (?, ?, 'comptoir_des_millesimes', ?, 'EUR', ?, ?, ?, ?, ?, 1)""",
