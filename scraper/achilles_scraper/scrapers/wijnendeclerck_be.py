...
"""
Wijnendeclerck.be scraper — retail price ingestion via HTML catalog.

Wijnen De Clerck is a Belgian wine shop (Dutch/French-language) with a
web shop at webshop.wijnendeclerck.be. Standard paginated catalog.
"""
import hashlib
import json
import logging
...
_BASE = "https://webshop.wijnendeclerck.be"
# Try common catalog slugs; scraper will fall back gracefully if 404
_CATALOGUE_PATHS = ["/wines", "/wijnen", "/vins", "/shop", "/"]
...
def _find_catalogue_url(client: "httpx.Client", base: str, paths: list) -> Optional[str]:
    """Try each path and return the first that returns HTTP 200 with product content."""
    for path in paths:
        url = f"{base}{path}"
        try:
...
class WijnendeclerckBeScraper(BaseScraper):
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"wijnendeclerck_be-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            ...
            while True:
                ...
                if resp.status_code in (403, 429):
                    msg = f"Blocked by wijnendeclerck.be: HTTP {resp.status_code} on {url}"
                ...
                for card in product_cards:
                    ...
                    try:
                        inserted = insert_staging_candidate(
                            self.conn,
                            wine_key=wine_key, source_key=SOURCE_KEY, retailer="wijnendeclerck_be",
