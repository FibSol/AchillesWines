...
"""
Lavinia.fr scraper — auth-gated retail price ingestion.

Lavinia is a French wine retailer using a Magento-style catalogue.
Products live in a paginated grid behind a login form. This scraper
authenticates via the Magento loginPost endpoint and crawls the catalogue.
"""
import hashlib
import json
import logging
...
_BASE = "https://www.lavinia.fr"
_LOGIN_URL = f"{_BASE}/customer/account/loginPost/"
...
class LaviniaScraper(AuthenticatedScraper):
    source_code = "lavinia"
    ...
    def _login(self, client: "httpx.Client", creds: "Credentials") -> bool:
        # First GET the login page to retrieve any hidden form tokens (CSRF)
        login_page_url = f"{_BASE}/customer/account/login/"
        resp_get = client.get(login_page_url, headers={"User-Agent": _USER_AGENT})

        form_key = ""
        if resp_get.status_code == 200:
            tree = HTMLParser(resp_get.text)
            form_key_node = tree.css_first("input[name='form_key']")
            if form_key_node:
                form_key = form_key_node.attrs.get("value", "")
        ...
        if resp.status_code in (200, 302):
            ...
            if "mot de passe" in text_lower and "incorrect" in text_lower:
                return False
            if "invalid" in text_lower and "password" in text_lower:
                return False
            return True
        return False
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        if not has_credentials(self.source_code):
            return ScrapeResult(
                error="Credentials missing: set ACHILLES_AUTH_LAVINIA_USERNAME / _PASSWORD"
            )

        batch_id = self.batch_id or f"lavinia-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        try:
            with self.authenticated_client(headers=headers, conn=self.conn) as client:
                ...
                while True:
                    ...
                    products = tree.css(".products-grid .product-item") or tree.css(".product-item") or []
                    ...
                    for node in products:
                        ...
                        name_node = node.css_first(".product-item-link, .product-name a, h2, h3")
                        price_node = node.css_first(".price, .product-price, .regular-price")
                        ...
                        appellation_node = node.css_first(".appellation, .wine-appellation, .category")
                        if appellation_node:
                            appellation = appellation_node.text(strip=True)
                            region = appellation

                        link_node = name_node if name_node.tag == "a" else node.css_first("a[href]")
