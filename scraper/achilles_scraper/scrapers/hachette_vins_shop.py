...
"""
Hachette-Vins.shop scraper — retail price ingestion via HTML catalog.

Hachette Vins Shop is the e-commerce arm of the Hachette wine guide.
Standard WooCommerce/Shopify-style paginated catalog at /vins.
"""
import hashlib
import json
import logging
...
class HachetteVinsShopScraper(BaseScraper):
    source_code = "hachette_vins_shop"
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"hachette_vins_shop-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            ...
            while True:
                ...
                if resp.status_code in (403, 429):
                    msg = f"Blocked by hachette-vins.shop: HTTP {resp.status_code} on {url}"
