...
"""
vinsbrunin.com scraper — public WooCommerce shop retail price ingestion.

Vins Brunin is a smaller French wine retailer running a WooCommerce
storefront. Products are publicly accessible and follow standard WooCommerce
HTML patterns.
"""
import hashlib
import json
...
_CATALOGUE_URL = f"{_BASE}/boutique"
...
def _parse_price(raw: str) -> Optional[float]:
    """Extract numeric price from WooCommerce price strings like '12,50 €' or '12.50'."""
    # WooCommerce may show "12,50 € – 45,00 €" for variable products — take first
    price_part = (raw or "").split("–")[0].split("-")[0].strip()
    cleaned = re.sub(r"[^\d.,]", "", price_part)
...
class VinsBruninScraper(BaseScraper):
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"vinsbrunin-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            while True:
                # WooCommerce pagination: /boutique/page/2/ or ?paged=2
                ...
                if resp.status_code in (403, 404, 429):
                    ...
                    msg = f"Blocked/not-found at vinsbrunin.com: HTTP {resp.status_code} on {url}"
                ...
                # Standard WooCommerce product list selectors
                products = (
                    tree.css("li.product")
                    or tree.css(".products .product")
                    or tree.css(".woocommerce ul.products li")
                    or []
                )
                ...
                for node in products:
                    ...
                    # WooCommerce standard title and price selectors
                    name_node = node.css_first(
                        ".woocommerce-loop-product__title, h2.product_title, h2, h3"
                    )
                    ...
                    # WooCommerce category tags sometimes expose appellation info
                    category_node = node.css_first(".product-category, .cat-label, .posted_in a")
                    if category_node:
                        appellation = category_node.text(strip=True)
                        region = appellation

                    link_node = node.css_first("a.woocommerce-loop-product__link, a[href]")
                ...
                # WooCommerce next-page detection
                next_node = tree.css_first(
                    "a.next.page-numbers, .woocommerce-pagination a.next, a[rel='next']"
                )
                if not next_node:
                    break
