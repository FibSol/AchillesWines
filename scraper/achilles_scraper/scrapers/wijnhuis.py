...
"""
Wijnhuis.be scraper — retail price ingestion via HTML catalog.

Wijnhuis is a Dutch-language Belgian wine shop running WooCommerce or
similar. Products are listed at /wijnen with standard paginated cards.
"""
import hashlib
import json
import logging
...
class WijnhuisScraper(BaseScraper):
    ...
    def run(self, limit: Optional[int] = None) -> ScrapeResult:
        ...
        batch_id = self.batch_id or f"wijnhuis-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
        ...
        with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
            ...
            while True:
                ...
                if resp.status_code in (403, 429):
                    msg = f"Blocked by wijnhuis.be: HTTP {resp.status_code} on {url}"
                ...
                for card in product_cards:
                    ...
                    try:
                        inserted = insert_staging_candidate(
                            self.conn,
                            wine_key=wine_key, source_key=SOURCE_KEY, retailer="wijnhuis",
